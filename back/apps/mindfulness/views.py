...
def complete_meditation(request):
    ...
    # Accept exact seconds from frontend; fall back to minutes if old client
    duration_seconds = int(data.get("duration_seconds", 0))
    if duration_seconds == 0:
        # Legacy: frontend sent duration_minutes
        legacy_minutes = int(data.get("duration_minutes", 0))
        duration_seconds = legacy_minutes * 60

    # Derive minutes — ceiling so 61s counts as 2 min, 60s as 1 min
...
# ---------------------------------------------------------
#  HEATMAP — CURRENT MONTH (or ?year=YYYY&month=M)
# ---------------------------------------------------------
@jwt_auth
def meditation_heatmap(request):
    ...
    # Fill all days of the month (missing = 0)
    result = {}
    d = first_day
    while d <= last_day:
...
def _update_user_meditation_stats(user_id):
    ...
    # ---- CURRENT STREAK ----
    # Count backwards from today; if user hasn't meditated today yet,
    # start from yesterday so an existing streak doesn't break mid-day.
    today = timezone.localdate()
    check_day = today if str(today) in meditated_days else today - timedelta(days=1)
    current = 0

    while True:
        if str(check_day) in meditated_days:
            current += 1
